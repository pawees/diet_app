...
class PreRequestWidget extends StatelessWidget {
  const PreRequestWidget({Key? key, required this.order, required this.date})
      : super(key: key);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _Agency(
              title: 'ДДЦ Центр здоровья для детей',
              address:
                  '610027,Кировская область,г.Киров,ул Красноармейская,43'),
          SizedBox(
            height: 20,
          ),
          Divider(
            ...
            child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _BuildOrderWidget(
                      data: order.places[index], selected: index);
                },
...
class _BuildOrderWidget extends StatelessWidget {
  const _BuildOrderWidget({Key? key, this.data, this.selected})
      : super(key: key);
  final data;
  final selected;
  @override
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: <Widget>[
        Text(data.name, style: h18_500()),
        SizedBox(height: 24),
        for (var item in data.diets)
          Column(
            children: [
              SizedBox(
                height: 16,
